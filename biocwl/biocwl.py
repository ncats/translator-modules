...
class BiolinkWorkflow(object):
    ...
    def _valid_output(self, output):
        if self._check(output, self.spec['output_type']):
            return True
        else:
            return False
...
def biolink_object_factory(spec_obj, input):
    ...
    data_type = "".join(" ".join(spec_obj.split("_")).title().split(" "))  # TODO: guarantee camelcase? would imply guaranteeing a certain format for self.spec (need spaces)
    biolink_class = dict(inspect.getmembers(biolink_model, inspect.isclass))[data_type]
    biolinkObj = biolink_class

    for key in input:
        setattr(biolinkObj, key, input)

    return biolinkObj


if __name__ == '__main__':
    fire.Fire(BiolinkWorkflow)
