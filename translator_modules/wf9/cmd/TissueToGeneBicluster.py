...
from translator_modules.wf9.util.bicluster_tissue_to_gene import tissue_to_gene
from translator_modules.core.module_payload import Payload


class TissueToGeneBicluster(Payload):

    def __init__(self, input_tissue):
        self.mod = TissueCoocurrenceWithGenesBicluster = tissue_to_gene()
        input_str, extension = self.handle_input_or_input_location(input_tissue)

        input_tissue_ids: list
        # NB: push this out to the handle_input_or_input_location function?
        if extension == "csv":
            import csv
            with open(input_tissue) as genes:
