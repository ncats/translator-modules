...
class CooccurrenceByBicluster():
    ...
    async def gene_to_gene_biclusters(self, curated_ID_list):
            ...
            for response in await asyncio.gather(*futures_1):
                ...
                if length_response_json > 0:
                    ...
                    with concurrent.futures.ProcessPoolExecutor(max_workers=2) as executor_2:
                        ...
                        related_biclusters_and_genes_for_each_input_gene[gene] = dict(cooccurrence_dict_each_gene)
                    # END COMPUTE related_biclusters_and_genes_for_each_input_gene

        return related_biclusters_and_genes_for_each_input_gene

    # TODO: not yet completed
    @UserWarning
    async def gene_to_tissue_biclusters(self, curated_ID_list):
        bicluster_url_list = [bicluster_gene_url + gene + '/' + '?include_similar=true' for gene in curated_ID_list]
        length_bicluster_url_list = len(bicluster_url_list)
            ...
            for response in await asyncio.gather(*futures_1):
                ...
                if length_response_json > 0:
                    ...
                    with concurrent.futures.ProcessPoolExecutor(
                            max_workers=cooccurrence_dict_each_gene['number_of_related_biclusters'] / 2) as executor_2:
                        loop_2 = asyncio.get_event_loop()
                        ...
                        for response_2 in await asyncio.gather(*futures_2):
                            ...
                            biclusterindex = [x['bicluster'] for x in response_2_json]
                            cooccurrence_dict_each_gene['related_biclusters'][
                                biclusterindex[0]] = tissues_in_each_bicluster
                        related_biclusters_and_tissues_for_each_input_gene[gene] = dict(cooccurrence_dict_each_gene)

        return related_biclusters_and_tissues_for_each_input_gene

    async def tissue_to_gene_biclusters(self, curated_ID_list):
                ...
                if length_response_json > 0:
                    ...
                    with concurrent.futures.ProcessPoolExecutor(max_workers=1) as executor_2:
                        ...
                        for response_2 in await asyncio.gather(*futures_2):
                            ...
                            cooccurrence_dict_each_gene['related_biclusters'][
                                biclusterindex[0]] = genes_in_each_bicluster
                        related_biclusters_and_genes_for_each_input_tissue[gene] = dict(cooccurrence_dict_each_gene)

        return related_biclusters_and_genes_for_each_input_tissue

    # TODO: not yet completed
    async def tissue_to_tissue_biclusters(self, curated_ID_list):
        bicluster_url_list = [bicluster_tissue_url + tissue + '/' + '?include_similar=true' for tissue in
                              curated_ID_list]
        length_bicluster_url_list = len(bicluster_url_list)
        with concurrent.futures.ProcessPoolExecutor(max_workers=1) as executor_1:
            ...
            futures_1 = [loop_1.run_in_executor(executor_1, requests.get, request_1_url) for request_1_url in
                         bicluster_url_list]
            for response in await asyncio.gather(*futures_1):
                cooccurrence_dict_each_tissue = defaultdict(dict)
                cooccurrence_dict_each_tissue['related_biclusters'] = defaultdict(dict)
                response_json = response.json()
                length_response_json = len(response_json)
                cooccurrence_dict_each_tissue['number_of_related_biclusters'] = length_response_json
                if length_response_json > 0:
                    tissue = response_json[0]['all_col_labels']
                    for x in response_json:
                        bicluster = x['bicluster']
                        cooccurrence_dict_each_tissue['related_biclusters'][x['bicluster']] = []
                    related_biclusters = [x for x in cooccurrence_dict_each_tissue['related_biclusters']]
                    bicluster_bicluster_url_list = [bicluster_bicluster_url + related_bicluster + '/' for
                                                    related_bicluster in related_biclusters]
                    ...
                    with concurrent.futures.ProcessPoolExecutor(max_workers=1) as executor_2:
                        ...
                        for response_2 in await asyncio.gather(*futures_2):
                            ...
                            tissues_in_each_bicluster = [bicluster['all_col_labels'] for bicluster in response_2_json]
                            biclusterindex = [x['bicluster'] for x in response_2_json]
                            cooccurrence_dict_each_tissue['related_biclusters'][
                                biclusterindex[0]] = tissues_in_each_bicluster
                        related_biclusters_and_tissues_for_each_input_tissue[tissue] = dict(
                            cooccurrence_dict_each_tissue)
    ...
    def tissues_in_unique_biclusters(self, list_of_unique_biclusters,
                                     related_biclusters_and_tissues_for_each_input_gene):
    ...
    def sorted_list_of_output_tissues(self, dict_of_ids_in_unique_biclusters_not_in_inputs):
        sorted_list_of_output_tissues = sorted(
            (value, key) for (key, value) in dict_of_ids_in_unique_biclusters_not_in_inputs.items())
